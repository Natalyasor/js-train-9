...
function findLastEvenNumber(arr) {
  ...
  const lastEvenNumber = arr.findLast((element) => {
    return element % 2 === 0;
  })
...
function getPositiveArrayValues(arr) {
  ...
  const positiveArrayValues = arr.filter(num => num > 0);
...
function sumAndPrint(arr) {
  ...
  const sum = arr.forEach(function(elem){
    console.log(elem);
  });
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  // За допомогою методу map() підносимо кожен елемент масиву до квадрату.
   const arr1 = Newarr.map(element => element * element);
  // За допомогою методу filter() фільтруємо елементи, які більше за порогове значення.
  const filteredSquaredArray = arr1.filter(elements => elements > threshold);
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  const sum = arr1.reduceRight((accumulator, currenValue) => accumulator + currenValue, 0);
...
function wordLengths(arr) {
  ...
  const arr1 = arr.map(word => word.length);
...
function expandArrayByN(arr, n) {
  ...
  if(!Array.isArray(arr) || typeof n !== 'number') {
    return "Помилка: Перший вхідний параметр має бути масивом, другий - числом"
  } ;
  // Використовуємо метод flatMap для створення нового масиву, де кожне число повторюється n разів
const arr1 = arr.flatMap((num) => Array(n).fill(num));
